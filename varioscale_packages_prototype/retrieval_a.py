...
class DataRetriever(object):
    ...
    def retrieve_data(self, bbox):
        ...
        #            where=edge_where)
        command = """
          SELECT geometry FROM {dataset}_tgap_edge WHERE {where}
        """.format(dataset=self.dataset, where=edge_where)
        with get_db_cursor() as cursor:
            cursor.execute(command)
            timer.measure('edge_query')
        ...
        # for geom_wkb, in resultset:
        #     edges["coordinates"].append(as_python_linestring(geom_wkb))
